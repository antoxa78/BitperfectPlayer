...
void destruct(thread *off_course)
{
    ...
    /* bionic libc has no pthread_cancel(). Nothing in the SACD bridge
       build calls ThreadDestroy() (it is a pigz leftover), so cancellation
       is skipped and the thread is joined normally. */
    ret = 0;
#else
