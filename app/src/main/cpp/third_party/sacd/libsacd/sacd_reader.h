...
/**
 * Opens an SACD image supplied through callback-based block access
 * (useful for streaming sources such as SMB).  `read` must return the
 * number of bytes read at `offset`, `size` the total image size in bytes.
 *
 * @return If successful a read handle is returned. Otherwise 0 is returned.
 */
sacd_reader_t *sacd_open_cb(sacd_input_read_fn read, sacd_input_size_fn size, void *opaque);
