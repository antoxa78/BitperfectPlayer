...
/* ------------------------------------------------------------------ */
/* Callback-backed image reader.                                       */
/* One slot per concurrent callback device; slots are keyed by fd value */
/* and dispatched only when dev->is_cb is set, so they never collide    */
/* with real file/socket descriptors.                                  */
/* ------------------------------------------------------------------ */
#define SACD_CB_MAX_SLOTS 32

typedef struct sacd_cb_slot_s
{
    int                  in_use;
    sacd_input_read_fn   read;
    sacd_input_size_fn   size;
    void                *opaque;
} sacd_cb_slot_t;

static sacd_cb_slot_t g_cb_slots[SACD_CB_MAX_SLOTS];
static pthread_mutex_t g_cb_lock = PTHREAD_MUTEX_INITIALIZER;

static int sacd_alloc_cb_slot(sacd_input_read_fn read, sacd_input_size_fn size, void *opaque)
{
    pthread_mutex_lock(&g_cb_lock);
    for (int i = 0; i < SACD_CB_MAX_SLOTS; i++)
    {
        if (!g_cb_slots[i].in_use)
        {
            g_cb_slots[i].in_use = 1;
            g_cb_slots[i].read = read;
            g_cb_slots[i].size = size;
            g_cb_slots[i].opaque = opaque;
...
void sacd_input_setup_cb(void);

static sacd_input_t sacd_dev_input_open   (const char *);
static int          sacd_dev_input_close   (sacd_input_t);
static uint32_t     sacd_dev_input_read    (sacd_input_t, uint32_t, uint32_t, void *);
static char        *sacd_dev_input_error   (sacd_input_t);
static int          sacd_dev_input_authenticate(sacd_input_t);
static int          sacd_dev_input_decrypt (sacd_input_t, uint8_t *, uint32_t);
static uint32_t     sacd_dev_input_total_sectors(sacd_input_t);

sacd_input_t sacd_input_open_cb(sacd_input_read_fn read, sacd_input_size_fn size, void *opaque)
{
    sacd_input_t dev;
    int slot;
    ...
    /* Make sure the global dispatcher pointer set below is installed. */
    sacd_input_setup_cb();

    slot = sacd_alloc_cb_slot(read, size, opaque);
    if (slot < 0)
        return NULL;
    ...
    if (!dev)
    {
        g_cb_slots[slot].in_use = 0;
        return NULL;
    }

    dev->fd = slot;
    dev->is_cb = 1;
    dev->input_buffer = NULL;

    return dev;
}

/**
 * Install the file / callback dispatcher function pointers (needed when no
 * device path - and hence no sacd_input_setup() call - is ever involved).
 */
void sacd_input_setup_cb(void)
{
...
static uint32_t sacd_cb_input_read(sacd_input_t dev, uint32_t pos, uint32_t blocks, void *buffer)
{
    ...
    if (!dev || dev->fd < 0 || dev->fd >= SACD_CB_MAX_SLOTS)
        return 0;
    s = &g_cb_slots[dev->fd];
    if (!s->in_use)
        return 0;

    off = (int64_t) pos * SACD_LSN_SIZE;
    want = (int) blocks * SACD_LSN_SIZE;
    n = s->read(s->opaque, off, buffer, want);
    if (n <= 0)
        return 0;

    return (uint32_t) n / SACD_LSN_SIZE;
}
...
{
    if (dev && dev->is_cb)
        return sacd_cb_input_read(dev, pos, blocks, buffer);
...
static int sacd_dev_input_close(sacd_input_t dev)
{
    ...
    if (dev && dev->is_cb)
    {
        if (dev->fd >= 0 && dev->fd < SACD_CB_MAX_SLOTS)
            g_cb_slots[dev->fd].in_use = 0;
        free(dev);
        return 0;
    }
...
static uint32_t sacd_dev_input_total_sectors(sacd_input_t dev)
{
    ...
    if (dev->is_cb)
    {
        if (dev->fd < 0 || dev->fd >= SACD_CB_MAX_SLOTS || !g_cb_slots[dev->fd].in_use)
            return 0;
        int64_t sz = g_cb_slots[dev->fd].size(g_cb_slots[dev->fd].opaque);
        return (sz <= 0) ? 0 : (uint32_t)(sz / SACD_LSN_SIZE);
    }
