...
/**
 * Open an SACD image through a callback-based block reader.
 */
sacd_reader_t *sacd_open_cb(sacd_input_read_fn read, sacd_input_size_fn size, void *opaque)
{
    ...
    dev = sacd_input_open_cb(read, size, opaque);
    if (!dev)
    {
        fprintf(stderr, "libsacdread: Can't open callback SACD reader\n");
        return NULL;
    }
