...
/*
 * Callback-backed SACD image reader (bridge to streaming sources such as
 * SMB).  `read` must return the number of bytes read starting at `offset`,
 * or <= 0 on error/EOF.  `size` returns the total image size in bytes.
 */
typedef int         (*sacd_input_read_fn)      (void *opaque, int64_t offset, void *buf, int len);
typedef int64_t     (*sacd_input_size_fn)      (void *opaque);
